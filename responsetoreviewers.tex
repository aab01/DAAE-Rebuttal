...
We now give motivation for reconstruction, we have included the following paragraph to justify these experiments:
...
\begin{itemize}
    ...
    \item {\color{red} The purpose of the image samples is to demonstrate the issue and demonstrate empiracly that our approach addresses the issue.}
\end{itemize}

{\color{red} \textbf{TIDY UP} In our revised paper, to empasise these points, we include empirical PDFs of distributions of encoded data samples, $q_\phi(z|x)$, encoded corrupted data samples $q_\phi(z|\tilde{x})$ and the prior $p(z)$, to shown the miss match between distributions, $q_\phi(z|x)$ and $p(z)$ when using the DAAE.}
...
{\color{red}
For the additional CelebA experiments, since an AAE had not been trained on the CelebA dataset using our proposed architecture, a more extensive hyper parameter search was performed. This is detailed in the revised paper as follows:
...
\begin{itemize}
    ...
    \item We have now done this and updated the reconstruction values which are shown in Table IV.
\end{itemize}
...
\begin{itemize}
    ...
    \item {\color{red} TO TIDY: Training time will vary based on resource available, choice of framework and machine. The additional computation cost from corruption is minimal, and the computational cost assocated with the adversarial loss is dependant on the size of the discriminator. Comparison of training time is likely to be less meaninful. Instead, we had added additional results to demonstate the benefits of using DAAE or iDAAE compared to other state of art models.}
\end{itemize}

In our revised version, we include facial attribute classification experiments which compare our model to state-of-the-art models rather than PCA, which we hope motivates the models further. Experiments are now comparable with very recent work in this field {\color{red} Citations needed here.}
...
\item In figure 1, on page 1 where we present our model, the caption in the original paper says:
\begin{quote}
...
\begin{itemize}
    ...
    \item At the start of section I.D. we are beginning to introduce the work of Im et al. Cited in paragraph two if Section I.D. who introduce DVAEs, VAE's with denoising.
\end{itemize}
...
\begin{itemize}
    ...
    \item In our revised version we refer to $v$ samples as ``input samples'' coming from our chosen prior distribution $p(v)$ and generated $w$ samples as ``output samples''. We in our revised version we say:
...
{\color{red} NEED TO UPDATE PAPER: For these experiments we use the same prior and so we now put this information in the caption as well as in the model description and remove it from the table.}
