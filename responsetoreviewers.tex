...
{\color{red} In our revised paper, we perform facial attribute classification on the CelebA dataset and compare to well known models including the VAE and $\beta$-VAE.}
...
{\color{red} We deliberately describe the method in a very general framework. The models $q_\phi(z|x)$ and $p_\theta(x|z)$ may be instatiated using any differentable parameterized model.}
...
{\color{red} TO TIDY: In our revised version we include facial attribute classification experients which compare our model to state of the art models rather than PCA, which we hope motivates the models futher.}
...
{\color{red} In our revised version we have performed additional experiments with the CelebA dataset of colour faces, perfroming facial attribute classification. We compare results to those obtained for well known models including VAE and $\beta$-VAE.}
...
{\color{red}
The first secntence is making a distinction between the reconstruction task (for which we no longer corrupt images) and the synthesis task. In the revised version of the paper we perform reconstruction on non-corrupted image, we have changed updates this sentence as follows:
